...
# Carregar documentos PDF (apenas se a pasta existir)
docs = []
docs_path = Path("assets/docs/")
if docs_path.exists():
...
# Criar Chunks + Embeddings (apenas se há documentos)
if docs:
...
def perguntar_politica_RAG(pergunta: str) -> Dict:
    if not retriever:
        return {"answer": "Sistema RAG não disponível.", "citacoes": [], "contexto_encontrado": False}
...
KEYWORDS_ABRIR_TICKET = ["aprovação", "execução", "liberação", "abrir ticket", "abrir chamado", "acesso especial"]
...
def processar_pergunta():
    try:
        ...
        # Processar com o grafo
        resultado = grafo.invoke({"pergunta": pergunta, "logs": []})
...
def status():
    return jsonify({
        'status': 'online',
        'documentos_carregados': len(docs),
        'rag_disponivel': retriever is not None
    })
